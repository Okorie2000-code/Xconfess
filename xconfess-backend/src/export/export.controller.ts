...
export class ExportController {
  ...
  async getExportJobs(@Param('userId') userId: string, @Req() req: any) {
    ...
    // DataExportService receives the verified caller ID — never trust the param alone.
    return this.dataExportService.getExportHistory(String(req.user.sub));
  }
  ...
  async downloadExportJob(
    ...
    return this.dataExportService.getExportFile(jobId, String(req.user.sub));
  }
}
